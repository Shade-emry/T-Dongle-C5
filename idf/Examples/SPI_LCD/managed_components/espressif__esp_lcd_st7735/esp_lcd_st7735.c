...
st7735_lcd_init_cmd_t st7735_init_cmds[] = {
    ...
    {ST7735_INVON,  (uint8_t[]){0x00}, 1, 0},                      // set inverted mode                                                                                          // Power control
    // {ST7735_INVOFF, (uint8_t[]){0x00}, 1, 0},                                                                                             // Don't invert display                                                                                            // Don't invert display
    {ST7735_COLMOD, (uint8_t[]){0x05}, 1, 0},                                                                                             // Set color mode (16-bit)
    // {ST7735_MADCTL, (uint8_t[]){0xE8}, 1, 0},                                                                                             // Set color mode (16-bit)
    {ST7735_GMCTRP1, (uint8_t[]){0x02, 0x1c, 0x07, 0x12, 
                                 0x37, 0x32, 0x29, 0x2d, 
                                 0x29, 0x25, 0x2B, 0x39, 
                                 0x00, 0x01, 0x03, 0x10}, 16, 0}, // Positive Gamma

    {ST7735_GMCTRN1, (uint8_t[]){0x03, 0x1d, 0x07, 0x06, 
                                 0x2E, 0x2C, 0x29, 0x2D, 
                                 0x2E, 0x2E, 0x37, 0x3F, 
                                 0x00, 0x00, 0x02, 0x10}, 16, 0}, // Negative Gamma
...
static esp_err_t panel_st7735_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    ESP_LOGI(TAG, "mirror_madctl_val=0x%x", st7735->madctl_val);
...
static esp_err_t panel_st7735_swap_xy(esp_lcd_panel_t *panel, bool swap_axes)
{
    ...
    // ESP_LOGI(TAG, "swap_xy_madctl_val=0x%x", st7735->madctl_val);
