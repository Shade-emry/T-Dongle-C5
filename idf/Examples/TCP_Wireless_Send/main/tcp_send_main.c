...
#define TCP_KEEPALIVE_IDLE 7200
#define TCP_KEEPALIVE_INTERVAL 75
...
{
    char host_ip[] = "192.168.3.169";  // 替换为服务器的实际IP地址
    ...
    while (1) {
        ...
        // 发送数据
        char *tx_buffer = heap_caps_malloc(TCP_RECV_BUFFER_SIZE, MALLOC_CAP_DEFAULT);
        if (tx_buffer == NULL) {
            ESP_LOGE(TAG, "Memory allocation failed for tx_buffer");
            close(sock);
            vTaskDelay(1000 / portTICK_PERIOD_MS);
            continue;
        }
        ...
        // strcpy(tx_buffer, send_data);

        while (1) {
            snprintf(tx_buffer, TCP_RECV_BUFFER_SIZE, 
                "Client RSSI: %d", 
                get_rssi());
...
void app_main(void)
{
    ...
    xTaskCreate(tcp_client_task, "WIFI TCP Server Task", 4096, NULL, 3, NULL);
}
